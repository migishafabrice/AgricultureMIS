...
String code="",fileName="",fieldName="",fuser="",luser="",sexuser="",muser="",tuser="",functuser="",url="",fid="",pass="",rpass="",prov="",dis="";
 SimpleDateFormat dte;
                ...
                Date dt=new Date();
rs=st.executeQuery("select * from stafftb order by id desc limit 1");
    if(rs.first())
    {
code=(Integer.valueOf(rs.getString("id"))+1)+"-STF-"+dte.format(dt);
...
String assign="";
PreparedStatement pt=con.prepareStatement("INSERT INTO `stafftb`(`id`, `staffid`, `name`, `surname`, `sex`, `telephone`, `email`, `position`,`assignedto`,`password`, `registrar`, `date`,`active`,`profilepic`)"
        + "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
pt.setString(1,null);
pt.setString(2, code);
...
pt.setString(6, tuser);
pt.setString(7, muser);
pt.setString(8, fid);
...
{
pt.setString(9, prov+"/"+dis);
assign=prov+"/"+dis;
}
pt.setString(10, pass);
pt.setString(11, session.getAttribute("userCode").toString());
dte=new SimpleDateFormat("dd-MM-yyyy");
pt.setString(12, dte.format(dt));
pt.setString(13, "YES");
pt.setString(14, url);
pt.execute();
...
String sms="Dear "+ fuser+" "+luser+
        ";<br/> You have been registered to E-Agriculture MIS as "+fid+ " of "+assign+
