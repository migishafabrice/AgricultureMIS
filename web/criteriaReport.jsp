...
//1
                        ...
                        try
                        {
                         ...
                         if(!session.getAttribute("userFunction").equals("Farmer"))
                         {
                             ...
                             {
                               rrs=rst.executeQuery("select * from criteriastb order by title asc,year desc");  
                             }
                             else{
                                 
                          rrs=rst.executeQuery("select * from criteriastb where location='"+session.getAttribute("userLocation")+"' order by title asc,year desc");
                             }
                          if(rrs.first())
                          {
                              rrs.beforeFirst();
...
document.add(new Paragraph("List of infrastructure required",subfont));
//document.add(new Paragraph("\n"));
document.add(new Paragraph("\n"));
PdfPTable table = new PdfPTable(7);
table.setWidthPercentage(100);
table.addCell(getNormalCell("#","czech",11));
table.addCell(getNormalCell("Title","czech",11));
table.addCell(getNormalCell("Description","czech",11));
table.addCell(getNormalCell("Location","czech",11));
table.addCell(getNormalCell("Prioty","czech",11));
...
table.addCell(getNormalCell(rrs.getString("title"),"greek",10));
table.addCell(getNormalCell(rrs.getString("description"),"greek",10));
table.addCell(getNormalCell(rrs.getString("location"),"greek",10));
table.addCell(getNormalCell(rrs.getString("priority"),"greek",10));
